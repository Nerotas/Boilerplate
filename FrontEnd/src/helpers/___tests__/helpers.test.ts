...
import * as BulkValidators from '../BulkValidators';
import * as csvConvertors from '../csvConvertors';
...
describe('BulkValidators', () => {
    it('bulkProductsValidation returns valid and invalid rows', () => {
        const products: CreateProduct[] = [
            {
                name: 'ValidProduct',
                defaultClientName: 'Valid',
                productSection: { name: 'Section', defaultClientName: 'Valid' },
                productTypes: [],
                synonyms: [],
                personTypes: [],
            },
        ];
        const productSections = [{ name: 'Section', defaultClientName: 'Valid' }];
        const productTypes = [{ name: 'Product', displayName: 'Product' }];
        const personTypes = [{ name: 'Person', displayName: 'Person' }];
        const result = BulkValidators.bulkProductsValidation(products, productSections, productTypes, personTypes);
        expect(result.validRows.length + result.invalidRows.length).toBe(products.length);
    });
});

describe('csvConvertors', () => {
    it('convertIndexCSVProductsToCreateProducts returns array', () => {
        const input = [
            {
                values: {
                    name: 'Product',
                    ...
                    defaultProductSection: 'DefSection',
        ...
        const result = csvConvertors.convertIndexCSVProductsToCreateProducts(input as any);
...
describe('DataGridHelpers', () => {
    ...
    it('getCreated returns formatted string', () => {
        expect(typeof DataGridHelpers.getCreated('', { createdOnUtc: '2022-01-01', createdByEmail: 'test@test.com' })).toBe('string');
    });
});

describe('loginHelpers', () => {
    it('validateMSALToken returns a JWT_TOKEN_STATUS', () => {
        expect(Object.values(loginHelpers.JWT_TOKEN_STATUS)).toContain(loginHelpers.validateMSALToken());
    });

    it('clearAllDataOnUserLogout clears sessionStorage', () => {
        sessionStorage.setItem('test', 'value');
        loginHelpers.clearAllDataOnUserLogout();
